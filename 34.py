...
app.layout = html.Div([dcc.Dropdown(id='cnt_dd', options=[{'label':i, 'value':i} for i in cnt_ls], style=dd1_style),
                       ...
                       dcc.Dropdown(id='t_hour', options=[{'label':str(i).zfill(2), 'value':str(i).zfill(2)} for i in range(24)]),
                       dcc.Dropdown(id='t_minute', options=[{'label':str(i).zfill(2), 'value':str(i).zfill(2)} for i in range(60)]),
...
@app.callback(
    ...
    Input(component_id='inp_time', component_property='date'),
...
def update_plot(t, inp_city, inp_mag_max):
    ...
    if t:
        ...
        time = datetime.strptime(t, '%Y-%m-%d') # movaqat
